...
def WriteToCompras(weather_data, mes, total, town=None):
    ...
    title_text = u"{0}".format(ugettext("LIBRO DE COMPRAS"))
    # merge cells
    ...
    worksheet_s.merge_range('J4:L4', ugettext("NIT.: 1021325022"), cell2)
    worksheet_s.merge_range('J5:L5', ugettext("CALLE ESTEBAN ARCE"), cell_center2)
    ...
    worksheet_s.write(6, 4, ugettext("Factura"), header)
    worksheet_s.write(6, 5, ugettext("No Autorizacion"), header)
    worksheet_s.write(6, 6, ugettext("Codigo de Control"), header)
    worksheet_s.write(6, 7, ugettext("Total factura"), header)
    worksheet_s.write(6, 8, ugettext("ICE"), header)
    worksheet_s.write(6, 9, ugettext("Excentos"), header)
    worksheet_s.write(6, 10, ugettext("Neto"), header)
    worksheet_s.write(6, 11, ugettext("Credito Fiscal"), header)
    ...
    for idx, data in enumerate(weather_data):
        ...
        worksheet_s.write_string(row, 6, data.cod_control, cell)
        worksheet_s.write_number(row, 7, data.total, cell_center)
        worksheet_s.write_number(row, 8, data.ice, cell_center)
        worksheet_s.write_number(row, 9, data.excentos, cell_center)
        worksheet_s.write_number(row, 10, neto, cell_center)
    ...
    row = row + 1
    worksheet_s.write(row, 6, ugettext("TOTAL"), cell_total)
    # worksheet_s.write_formula(row, 3, '=SUM(D6:D7)')
    worksheet_s.write(row, 7, total, cell_total)
    worksheet_s.write(row, 8, ugettext("-"), cell_total)
    worksheet_s.write(row, 9, ugettext("-"), cell_total)
    worksheet_s.write(row, 10, neto_total, cell_total)
