...
from apps.bancarizacion.models import BancarizacionCompras


def WriteToBancarizacionCompras(weather_data, mes, total, town=None):
    ...
    worksheet_s.write(0, 0, ugettext("Fecha de compra"), header)
    ...
    worksheet_s.write(0, 3, ugettext("Fecha fact. DUI/Fecha Doc."), header)
    ...
    worksheet_s.write(0, 7, ugettext("NIT Proveedor"), header)
    worksheet_s.write(0, 8, ugettext("NIT Entidad Financiera"), header)
    worksheet_s.write(0, 9, ugettext("Razon Social Proveedor"), header)
    worksheet_s.write(0, 10, ugettext("Nro. de Documento de pago"), header)
    worksheet_s.write(0, 11, ugettext("Nro. De fact. DUI/Nro. Doc."), header)
    worksheet_s.write(0, 12, ugettext("Tipo de Doc. de pago"), header)
    worksheet_s.write(0, 13, ugettext("Tipo de documento"), header)
    worksheet_s.write(0, 14, ugettext("Monto fact./Monto DUI/Monto Doc."), header)
    worksheet_s.write(0, 15, ugettext("Fecha del documento de pago"), header)
    worksheet_s.write(0, 16, ugettext("Nro. de Aut. Fact./ DUI/Documento"), header)
    ...
    for idx, data in enumerate(weather_data):
        ...
        worksheet_s.write_number(row, 7, data.nit_proveedor, cell_center)
        worksheet_s.write_number(row, 8, data.nit_entidad_inanciera, cell_center)
        worksheet_s.write_string(row, 9, data.razon_social_proveedor, cell_center)
        worksheet_s.write_number(row, 10, data.nro_d_documento_de_pago, cell_center)
        worksheet_s.write_number(row, 11, data.nro_d_fact_dui_nro_doc, cell_center)
        ...
        worksheet_s.write_string(row, 14, data.monto_fact_monto_dui_monto_doc, cell_center)
        worksheet_s.write_string(row, 15, data.fecha_d_documento_d_pago.strftime('%d/%m/%Y'), cell_center)
        worksheet_s.write_number(row, 16, data.nro_de_aut_fact_dui_documento, cell_center)
